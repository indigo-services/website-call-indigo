...
async function pushDatabaseDirectly() {
  ...
  try {
    ...
    const methods = [
      {
        ...
        command: `cat /tmp/production-content.db | ssh root@vps10.riolabs.ai "docker exec -i indigo-studio sh -c 'cat > /app/.tmp/data.db'"`
      },
      {
        name: 'Method 2: Direct Volume Copy',
        command: `scp /tmp/production-content.db root@vps10.riolabs.ai:/etc/easypanel/projects/riostack/indigo-studio/code/strapi/.tmp/data.db`
      },
      {
        name: 'Method 3: Docker Volume Mount',
        command: `ssh root@vps10.riolabs.ai "docker cp - /tmp/production-content.db indigo-studio:/app/.tmp/data.db"`
      }
    ];
    ...
    console.log('1. Access Easypanel web interface: https://vps10.riolabs.ai');
    ...
    console.log('   scp /tmp/production-content.db root@vps10.riolabs.ai:/tmp/');
