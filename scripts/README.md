...
This directory contains automated scripts for production deployment validation, health monitoring, CI/CD status tracking, and Easypanel operations.

## Easypanel Operations

The Easypanel surface now uses the live self-hosted API configured by `EASYPANEL_API_URL`, with `EASYPANEL_API` as the canonical token key and `EASYPANEL-API` as a backward-compatible fallback.

### Commands

- `yarn toolset:init` scaffolds ignored local env files from the tracked provider metadata.
- `yarn easypanel:init` remains as a compatibility wrapper around `toolset:init`.
- `yarn easypanel:doctor` checks config, auth, project lookup, OpenAPI discovery, and panel reachability.
- `yarn easypanel:status` lists project services for the configured Easypanel project and highlights the configured target service.
- `yarn easypanel:health` checks the frontend and Strapi URLs, and now emits explicit warnings when those public URLs are still unset.
- `yarn easypanel:cron` prints recommended Linux cron entries for periodic doctor, status, health, and recommendation runs.
- `yarn easypanel:recommend` prints runtime and Strapi-specific Easypanel recommendations.
- `yarn easypanel:bootstrap` creates or updates the configured Easypanel Strapi target service using the live TRPC API.
- `yarn deploy:easypanel:api` runs bootstrap and then triggers deployment for the configured target service.
- `yarn test:toolset` runs the focused env-meta and portable config tests for the shared toolset layer.

The portable `.env.local` template also reserves OpenAI-compatible provider placeholders such as `ZAI_API_KEY` and `ZAI_OPENAI_BASE_URL` for local AI tooling. The loader now honors `process.env`, so Windows user-level env vars can supply those keys without writing them into the repo.

### Config files

- `.env.example` is the portable root config template.
- `.env.easypanel.example` is the Easypanel control-plane template for the Strapi target service.
- `.env.easypanel.local` is the ignored local Easypanel override file generated by `yarn easypanel:init`.
- `.env.production.local` is the ignored local frontend/runtime override file generated by `yarn easypanel:init`.
- `.env.production` supplies Next.js runtime variables used during bootstrap.
- `strapi/easypanel.env` supplies Strapi runtime variables used during bootstrap.
- `docs/easypanel-ops.md` documents the current `Vercel frontend + Easypanel compose Strapi` topology.
- `TOOLSET_BACKLOG.md` captures deferred provider-neutral, secops, and interoperability work.
