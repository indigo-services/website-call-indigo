...
async function runProductionValidation() {
  ...
  try {
    ...
    const hasCorrectContext =
      composeContent.includes('context: .') ||
      composeContent.includes('context: "."');
    const hasCorrectDockerfile = composeContent.includes(
    ...
    check(
      ...
      hasCorrectContext,
      hasCorrectContext
        ? 'Using correct build path'
        : 'Build context may be wrong'
    );
    check(
      'Dockerfile path correct',
      hasCorrectDockerfile,
      hasCorrectDockerfile
        ? 'Using strapi/Dockerfile'
  ...
  check(
    '.env file exists',
    envExists,
    envExists ? 'Environment configuration found' : 'Missing .env file'
  );
  ...
  try {
    execSync('gh repo view indigo-services/indigo-studio', {
      encoding: 'utf-8',
      stdio: 'pipe',
    });
    check(
      ...
      true,
      'Can access indigo-services/indigo-studio'
    );
  } catch (error) {
    check(
      'GitHub repository accessible',
  ...
  try {
    const API_TOKEN =
      process.env.EASYPANEL_API ||
      'e590a9387b6628af8d14744eeb527e71ad394d7d66451b61bd046a7d17333172';
    const result = execSync(
      `curl -s -w "%{http_code}" -o /dev/null "https://vps10.riolabs.ai/api" -H "Authorization: Bearer ${API_TOKEN}"`,
      { encoding: 'utf-8' }
    );
    const apiAccessible = result.includes('200') || result.includes('401'); // 401 means API is up but auth issue
    check(
      'Easypanel API accessible',
      apiAccessible,
      apiAccessible
        ? 'Easypanel server is reachable'
        : 'Cannot reach Easypanel API'
    );
  } catch (error) {
  ...
  try {
    const deployKeys = execSync(
      'gh repo deploy-key list indigo-services/indigo-studio',
      { encoding: 'utf-8', stdio: 'pipe' }
    );
    const hasEasypanelKey =
      deployKeys.includes('d6357425ba02') || deployKeys.includes('easypanel');
    check(
      'Easypanel deploy key exists',
      hasEasypanelKey,
      hasEasypanelKey
        ? 'Deploy key found in GitHub'
        : 'Easypanel SSH key not added to GitHub'
    );
  } catch (error) {
  ...
  try {
    const response = await fetch(
      'https://riostack-indigo-studio.ck87nu.easypanel.host/',
      { method: 'HEAD' }
    );
    ...
    if (is200) {
      check(
        'Deployment status',
        true,
        'Service is live and functional (200 OK)'
      );
    } else if (is502) {
    ...
    check(
      ...
      false,
      `Cannot reach deployment: ${error.message}`
    );
  }
  ...
  if (checksFailed === 0) {
    ...
    if (!require('fs').existsSync('.env'))
      error('• Create .env file with proper configuration');
    if (execSync('git status --porcelain', { encoding: 'utf-8' }).trim() !== '')
      error('• Commit or stash changes');
